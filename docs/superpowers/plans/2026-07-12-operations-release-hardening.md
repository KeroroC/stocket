...
**Architecture:** Gateway 终止 HTTPS、提供静态 PWA 并只代理 `/api`；Spring Boot JVM 应用、PostgreSQL、附件和备份使用最小权限、独立持久卷与健康门禁。CI 将快速验证与发布验证分层，发布流程生成双架构镜像、SBOM、漏洞报告、版本清单和 SHA-256 校验和；恢复演练在全新临时环境中验证数据库、附件、迁移和库存一致性。

**Tech Stack:** Java 25、Spring Boot 4.0.3 Actuator/Micrometer、PostgreSQL 17、Docker Buildx、Docker Compose、Nginx、GitHub Actions、Trivy、Syft、Cosign、pg_dump/pg_restore、OpenTelemetry/Micrometer Observation
...
App 使用两阶段构建，运行镜像只包含 JRE、应用 JAR、CA 和非 root 用户；根文件系统只读，`/tmp` 使用 size-limited tmpfs。前端构建产物进入 Nginx 非 root 镜像。PostgreSQL 数据、附件和备份使用三个命名卷，app 只挂载附件卷，backup job 才同时挂载数据库连接、附件和备份卷。
...
PR workflow 并行执行 backend JVM tests、module verify、frontend tests/typecheck/build、migration test、Compose config 和 shell tests；使用 lockfile/Maven cache，权限默认 `contents: read`，不向 fork PR 注入 secrets。

- [ ] **Step 2：建立夜间深度验证**

nightly 执行 JVM 测试、Playwright、恢复演练、Trivy filesystem/image 扫描和 SBOM 生成。失败上传脱敏测试报告、日志和扫描 JSON，保留 14 天；不上传数据库、附件 fixture 中的真实数据或环境文件。
...
Run: `make test && make build && docker compose -f deploy/compose.yml -f deploy/compose.production.yml config --quiet`
...
## Task 7：实现 AMD64/ARM64 容器发布与供应链产物
...
只允许 `vMAJOR.MINOR.PATCH` tag 触发；tag 与 Maven/package 版本必须一致，commit 必须在 main 且工作树来源可追溯。构建并验证 AMD64 和 ARM64 多架构 JVM 容器镜像。

- [ ] **Step 2：构建和验证每个架构**

运行 JVM tests，构建 multi-arch OCI image，并记录 image digest、Java 版本和平台列表。

- [ ] **Step 3：生成 SBOM、签名与发布清单**

Syft 为 image 生成 SPDX JSON；Trivy 扫描；Cosign 使用 GitHub OIDC keyless 签名镜像和 attest SBOM/provenance。`release-manifest.json` 包含版本、commit、构建时间、平台、checksum、image digest、SBOM 和测试摘要；同时生成 `SHA256SUMS`。

- [ ] **Step 4：实现发布验证器**

`verify-release.sh` 校验 checksum、Cosign 签名、SBOM、manifest schema、平台列表和 tag/版本一致性。测试用篡改文件、缺失 digest、错误 digest 和过期漏洞豁免证明会失败。
...
git diff --cached --check
git commit -m "ci(release): 发布双架构容器产物"
```
...
Expected: JVM、前端、TypeScript、配置契约和普通构建全部 PASS。
...
本地验证 release tooling、checksum 和负向 fixture。镜像扫描、SBOM、签名和 attestation 在实际发布时执行；验收报告仅记录实际获得的 workflow run、commit、image digest、备份 ID、恢复时长和检查结果，不记录秘密或真实家庭数据。
