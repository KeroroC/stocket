...
<template><section class="attachment-section"><h2>图片</h2><p v-if="error" class="st-feedback st-feedback--error" role="alert">{{error}}</p><ul v-if="images.length" class="attachment-list"><li v-for="item in images" :key="item.id"><span>{{item.filename}}</span><span>{{item.purpose==='COVER_IMAGE'?'封面':'图片'}}</span><button class="st-button" type="button" @click="download(item)">下载 {{item.filename}}</button><button v-if="canWrite" class="st-button st-button--danger" type="button" @click="remove(item)">删除 {{item.filename}}</button></li></ul><p v-else>暂无图片</p><template v-if="canWrite"><AttachmentUploader :owner-type="ownerType" :owner-id="ownerId" purpose="ITEM_IMAGE" label="上传图片" @uploaded="added"/><AttachmentUploader :owner-type="ownerType" :owner-id="ownerId" purpose="COVER_IMAGE" label="替换封面" @uploaded="added"/></template></section></template>
