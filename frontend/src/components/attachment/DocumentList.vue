...
<template><section class="attachment-section"><h2>发票与保修</h2><p v-if="error" role="alert">{{error}}</p><ul v-if="documents.length" class="attachment-list"><li v-for="item in documents" :key="item.id"><span>{{item.filename}}</span><span>{{item.purpose==='INVOICE'?'发票':'保修'}}</span><button type="button" @click="download(item)">下载 {{item.filename}}</button><button v-if="canWrite" type="button" @click="remove(item)">删除 {{item.filename}}</button></li></ul><p v-else>暂无发票或保修文件</p><div v-if="canWrite" class="document-uploaders"><AttachmentUploader :owner-type="ownerType" :owner-id="ownerId" purpose="INVOICE" label="上传发票" @uploaded="added"/><AttachmentUploader :owner-type="ownerType" :owner-id="ownerId" purpose="WARRANTY" label="上传保修文件" @uploaded="added"/></div></section></template>
