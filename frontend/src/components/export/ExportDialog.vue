...
<template><div class="export-control"><button class="st-button" type="button" :disabled="pending" @click="run">{{pending?'正在导出':(label??'导出 CSV')}}</button><p v-if="error" class="st-feedback st-feedback--error" role="alert">{{error}}</p></div></template>
