...
describe('ReceiveWizardView', () => {
  ...
  it('无需扫码即可从有效位置列表选择存放位置', async () => {
    ...
    const locationField = screen.getByLabelText('存放位置')
    await fireEvent.click((locationField.querySelector?.('[role="combobox"]') ?? locationField) as HTMLElement)
    await fireEvent.click(await screen.findByRole('option', { name: '家 > 储物间 > 储物柜' }))
  ...
  it('手工搜索并选择物品和位置后可以确认入库', async () => {
    ...
    await fireEvent.input(screen.getByLabelText('搜索物品'), { target: { value: '牛奶' } })
    ...
    const locationField = screen.getByLabelText('存放位置')
    await fireEvent.click((locationField.querySelector?.('[role="combobox"]') ?? locationField) as HTMLElement)
    await fireEvent.click(await screen.findByRole('option', { name: '家 > 冰箱' }))
    await fireEvent.click(screen.getByRole('button', { name: '下一步' }))
    await fireEvent.click(screen.getByRole('button', { name: '确认入库' }))
  ...
  it('目录中没有物品时可以快速建档并完成首次入库', async () => {
    ...
    await fireEvent.click(screen.getByRole('button', { name: '手工选择' }))
    await fireEvent.input(screen.getByLabelText('搜索物品'), { target: { value: '燕麦奶' } })
    expect(await screen.findByText('没有找到物品')).toBeInTheDocument()
    await fireEvent.click(screen.getByRole('button', { name: '创建新物品' }))
    const itemNameField = screen.getByLabelText('物品名称')
    await fireEvent.input((itemNameField.querySelector?.('input') ?? itemNameField) as HTMLInputElement, { target: { value: '燕麦奶' } })
    const categoryField = screen.getByLabelText('分类')
    await fireEvent.click((categoryField.querySelector?.('[role="combobox"]') ?? categoryField) as HTMLElement)
    await fireEvent.click(await screen.findByRole('option', { name: '饮品' }))
    const unitField = screen.getByLabelText('默认单位')
    await fireEvent.input((unitField.querySelector?.('input') ?? unitField) as HTMLInputElement, { target: { value: '盒' } })
