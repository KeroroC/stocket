...
<template>
  <section class="st-page">
    <StPageHeader title="位置管理" description="位置用于记录物品存放处；创建位置时系统会自动生成唯一、稳定的位置码。">
      <template #actions>
        <el-button type="primary" @click="startCreating">{{ selected ? '添加子位置' : '创建位置' }}</el-button>
      </template>
    </StPageHeader>
    ...
    <StEmptyState v-else-if="!nodes.length" title="还没有位置" description="先创建第一个顶级位置，例如“家”或“仓库”。">
      <el-button type="primary" @click="startCreating">创建第一个位置</el-button>
    </StEmptyState>
    <div v-else class="admin-grid">
      ...
      <el-card v-else-if="selected" class="admin-detail" shadow="never">
        ...
        <el-button @click="copy">复制位置码</el-button>
      </el-card>
    </div>
  </section>
</template>
