...
<template><article v-if="item"><StPageHeader :title="item.name" eyebrow="物品详情"><template #actions><ExportDialog kind="catalog" :filters="{q:item.name}" label="导出当前物品"/><button v-if="canWrite" @click="archive">归档物品</button></template></StPageHeader><dl><dt>单位</dt><dd>{{item.defaultUnit}}</dd><dt>条码</dt><dd><span v-for="code in item.barcodes" :key="code">{{code}}</span></dd><dt>标签</dt><dd><span v-for="tag in item.tags" :key="tag">{{tag}}</span></dd></dl><AttachmentGallery owner-type="ITEM_DEFINITION" :owner-id="item.id" :can-write="canWrite"/><DocumentList owner-type="ITEM_DEFINITION" :owner-id="item.id" :can-write="canWrite"/></article><p v-else-if="error" role="alert">{{error}}</p><p v-else>正在加载物品...</p></template>
